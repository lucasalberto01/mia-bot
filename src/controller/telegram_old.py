...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    message = "Oii, Meu nome é Mia e eu estou aqui para conversar! Bora bater um papo cabeça"
...
async def thinking(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if (serve_id != None):
        if checkExistUserGroup(usuario_id, serve_id):
            ...
            # MANDA A MENSAGEM
            await context.bot.send_message(serve_id, response)
    ...
    for b in arr:
        if is_reply_of_me or b in ["mia", "Mia", "mia.", ",mia" "Mia,", "mia?", "Mia?"]:
            ...
            if ultima_mensagem(usuario_id, entrada, usuario_user, usuario_nome) == True:
                await bot.send_message(message.chat.id, '🌸 Oii, eu sou a Mia, e estou aqui para conversar! Bora bater um papo cabeça 🌸')
                return
                ...
                for i in txt.split('#'):
                    if i == 'sendimg':
                        ...
                        if primeira:
                            ...
                            await context.bot.send_message(serve_id, i)
            break
...
def main() -> None:
    ...
    application.add_handler(CommandHandler("publicar", publicar))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, thinking))
